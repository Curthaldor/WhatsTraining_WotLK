...
local ROW_HEIGHT =			14					-- Actual visual row height in pixels
local ROW_STEP = ROW_HEIGHT + 2					-- Effective vertical step between rows (height + gap)
...
function wt.Update(frame, forceUpdate)
	...
	FauxScrollFrame_Update(wt.MainFrame.scrollBar, #wt.data, MAX_ROWS,
					ROW_STEP, nil, nil, nil, nil, nil, nil, true)
	lastOffset = offset
end
...
--	hooksecurefunc("SpellBookFrame_UpdateSkillLineTabs", function()
	...
	-- Create search box background to hide the "Show all spell ranks" checkbox and underlay the label
	local searchBg = mainFrame:CreateTexture(nil, "BACKGROUND")
	searchBg:SetTexture(0, 0, 0, 1) -- Solid black background (R, G, B, A)
	searchBg:SetPoint("TOPLEFT", mainFrame, "TOPLEFT", 76, -36)
	searchBg:SetWidth(192)
	searchBg:SetHeight(24)
	
	-- Create invisible frame to block clicks to the checkbox underneath
	local clickBlocker = CreateFrame("Frame", nil, mainFrame)
	clickBlocker:SetPoint("TOPLEFT", mainFrame, "TOPLEFT", 66, -36)
	clickBlocker:SetWidth(214)
	clickBlocker:SetHeight(24)
	clickBlocker:SetFrameLevel(mainFrame:GetFrameLevel() + 1)
	clickBlocker:EnableMouse(true)
	clickBlocker:SetScript("OnMouseDown", function() end) -- Absorb clicks
	
	-- Search box label positioned to the left of the search box
	local searchLabel = mainFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
	searchLabel:SetPoint("TOPLEFT", mainFrame, "TOPLEFT", 77, -43)
	searchLabel:SetText("Search:")
	
	-- Create search box positioned over the "Show all spell ranks" checkbox
	local searchBox = CreateFrame("EditBox", "$parentSearchBox", mainFrame, "InputBoxTemplate")
	searchBox:SetHeight(20)
	searchBox:SetWidth(145)
	searchBox:SetPoint("TOPLEFT", mainFrame, "TOPLEFT", 121, -39)
	searchBox:SetFrameLevel(clickBlocker:GetFrameLevel() + 1) -- Ensure search box is above click blocker
	searchBox:SetAutoFocus(false)
	searchBox:SetMaxLetters(50)
	searchBox:SetScript("OnTextChanged", function(self)
		local text = self:GetText():lower()
		wt.searchFilter = text ~= "" and text or nil
		wt:RebuildData()
	end)
	...
	scrollBar:SetScript("OnVerticalScroll", function(self, offset)
		FauxScrollFrame_OnVerticalScroll(self, offset, ROW_STEP,
